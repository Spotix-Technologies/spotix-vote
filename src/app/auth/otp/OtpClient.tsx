...
/**
 * app/auth/otp/OtpClient.tsx
 *
 * Six single-digit boxes for entering the OTP a Supabase Auth email
 * sends. Reached from:
 *   - /auth/signup, right after supabase.auth.signUp() succeeds
 *   - /auth/login, when signInWithPassword() fails with "Email not
 *     confirmed" (loggedInWaitingOTP=true — same verification, just a
 *     different heading/copy since they already have a password set)
 *
 * ⚠️ Requires the Supabase project's "Confirm signup" email template to
 * actually send the 6-digit code — i.e. use {{ .Token }} in the
 * template body, NOT {{ .ConfirmationURL }} (the default magic-link
 * template). If the template still sends a link, there's no code for
 * anyone to type here and every attempt will correctly show "wrong
 * code" no matter what's entered, because nothing was ever issued to
 * match against. Set this under Supabase Dashboard → Authentication →
...
export function OtpClient() {
  ...
  return (
    <main className="mx-auto flex min-h-[calc(100vh-57px)] w-full max-w-sm flex-col items-center justify-center px-6 py-16 text-center">
      <div className="w-full rounded-2xl border border-line bg-ink-2/90 p-6 backdrop-blur-md sm:p-8">
        ...
        <div className="relative mt-12 flex h-64 w-full items-center justify-center">
          ...
          {phase !== "input" && (
            <div className="pointer-events-none absolute inset-0 flex items-center justify-center px-10">
              ...
              {phase === "success" && <p className="otp-center-text font-mono text-sm text-success">✓ Verified</p>}
              {phase === "error" && <p className="otp-center-text font-mono text-xs text-danger">Wrong code — try again</p>}
            </div>
          )}
        </div>

        {errorText && phase === "input" && <p className="mt-2 text-sm text-danger">{errorText}</p>}
        ...
        <div>
          <Link href="/auth/login" className="mt-4 inline-block text-sm text-muted hover:text-paper">
            Back to sign in
          </Link>
        </div>
      </div>
